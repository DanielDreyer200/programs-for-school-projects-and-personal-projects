...
void Game::ImproveWeapon(){
  ...
  if(exp < COST_IMPROVEMENT){
    ...
    //asks user for how many times they would like to  improve with
    //max they can, being 1 according to the sample files 
    cout<<"how much woudl you like to improve your weapon? (max 1)"<<endl;

    //gets the number made and sends to amount
    cin>>amount;


    //if amout is one increase weapon and tell user
    if(amount == 1){
      ...
      //if 0 does not increase and tells user
    }else if(amount == 0){

      cout<<"you have not improved your weapon"<<endl;

      //too low or too high of a number was given desplaying problem to
      //user and sends them to menu
    }else{
...
bool Game::ProcessAttack(bool enemyAlive, bool alive){
  ...
  if((m_myHero ->GetHealth() <1) && (m_curEnemy ->GetHealth() <0)){
      ...
      //return false and desplay to user they have died
    }else if (m_myHero ->GetHealth() <1){
